...
public class PageService {
     ...
     * @return
     */
    public Mono<PageCreateResponseDto> createPage(Long workspaceId, PageCreateRequestDto request) {
        // 클라이언트의 인증 처리
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                        ...
                        reactiveWorkspaceRepository.findById(workspaceId)
                                ...
                                .flatMap(workspace -> {
                                    ...
                                    return reactiveWorkspaceMemberRepository.existsByWorkspaceIdAndUserId(workspaceId, customUserDetails.getId())
                                            .flatMap(exists -> {
                                                if (!exists) {
                                                    return Mono.error(new WorkspaceException(ErrorCode.MEMBER_NOT_FOUND));
                                                }
                                                return Mono.just(customUserDetails);
                                            });
                                })
                                .flatMap(customUserDetailsAfterAccessCheck -> {
                                    ...
                                    return reactivePageRepository.findByIdAndWorkspaceId(request.getParentPageId(), workspaceId)
                                                    ...
                                                    reactivePagePermissionRepository.findByPageIdAndUserId(parentPage.getId(), customUserDetailsAfterAccessCheck.getId())
                                                            .flatMap(permission ->{
                                                                ...
                                                                if (permissionType.getLevel() >= PagePermissionType.EDIT.getLevel()) {
                                                                    ...
                                                                    return Mono.error(new PageException(ErrorCode.PARENT_PAGE_PERMISSION_DENIED));
                                                                }
                                                            })
                                                            .switchIfEmpty(Mono.error(new PageException(ErrorCode.PARENT_PAGE_PERMISSION_DENIED)))
                                            );
                                })
     ...
     * @return
     */
    public Mono<PageListResponseDto> getPages(Long workspaceId) {
        // 클라이언트의 인증 처리
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
     ...
     * @return
     */
    public Mono<PageResponseDto> getPage(Long workspaceId, Long pageId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                                                    ...
                                                    if (isMember) {
                                                        return reactivePagePermissionRepository.findByPageIdAndUserId(pageId, customUserDetails.getId())
                                                                .flatMap(permission -> {
                                                                    ...
                                                                    return Mono.error(new PageException(ErrorCode.PAGE_PERMISSION_DENIED));
                                                                })
                                                                .switchIfEmpty(Mono.error(new PageException(ErrorCode.PAGE_PERMISSION_DENIED)));
                                                    } else {
                                                        if (page.isPublic()) {
                                                            return Mono.just(page);
                                                        }
     ...
     * @return
     */
    public Mono<PageResponseDto> updatePage(Long workspaceId, Long pageId, PageUpdateRequestDto request) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
     ...
     * @return
     */
    public Mono<PageInviteResponseDto> inviteMemberToPage(Long workspaceId, Long pageId, PageInviteRequestDto request) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(clientUser ->
                        reactiveWorkspaceRepository.findById(workspaceId)
                                ...
                                .flatMap(workspace ->
                                        reactivePageRepository.findByIdAndWorkspaceId(pageId, workspaceId)
                                                ...
                                                .flatMap(page ->
                                                        // 6: 초대할 사용자가 워크스페이스 멤버인지 확인
                                                        reactiveWorkspaceMemberRepository.existsByWorkspaceIdAndUserId(workspaceId, request.getUserId())
                                                                .flatMap(isInvitedUserWorkspaceMember -> {
                                                                    ...
                                                                    return reactivePagePermissionRepository.findByPageIdAndUserId(pageId, request.getUserId())
    ...
    public Mono<PageUpdatePermissionResponseDto> updateMemberPagePermission(
            Long workspaceId, Long pageId, Long targetUserId, PageUpdatePermissionRequestDto request
    ) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                                                        ...
                                                        reactiveWorkspaceMemberRepository.existsByWorkspaceIdAndUserId(workspaceId, targetUserId)
                                                                .flatMap(isTargetUserWorkspaceMember -> {
                                                                    ...
                                                                    if (page.getCreatedBy().equals(targetUserId)) {
                                                                        return Mono.error(new PageException(ErrorCode.CANNOT_CHANGE_OWNER_PERMISSION));
                                                                    }
                                                                    // 8: 페이지 권한 수정
                                                                    return reactivePagePermissionRepository.findByPageIdAndUserId(pageId, targetUserId)
                                                                            ...
                                                                            .switchIfEmpty(Mono.error(new PageException(ErrorCode.PAGE_PERMISSION_NOT_FOUND)));
                                                                })
                                                )
                                )
                )
    ...
    public Mono<PagePublicStatusUpdateResponseDto> updatePagePublicStatus(
            Long workspaceId, Long pageId, PagePublicStatusUpdateRequestDto request
    ) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
     ...
     * @return
     */
    public Mono<PageStatusResponseDto> archivePage(Long workspaceId, Long pageId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                ...
                .flatMap(page ->
                        ...
                        reactivePageRepository.updateArchiveStatusForTree(pageId, true, page.getLastEditedBy())
                                ...
                                .thenReturn(PageStatusResponseDto.builder().pageId(pageId).isArchived(true).build())
                )
                .onErrorMap(throwable -> {
                    if (throwable instanceof PageException ||
                            throwable instanceof WorkspaceException ||
     ...
     * @return
     */
    public Mono<PageStatusResponseDto> restorePage(Long workspaceId, Long pageId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                ...
                .flatMap(page ->
                        ...
                        reactivePageRepository.updateArchiveStatusForTree(pageId, false, page.getLastEditedBy())
                                ...
                                .thenReturn(PageStatusResponseDto.builder().pageId(pageId).isArchived(false).build())
                )
                .onErrorMap(throwable -> {
                    if (throwable instanceof PageException ||
                            throwable instanceof WorkspaceException ||
     ...
     * @return
     */
    public Mono<Void> deletePage(Long workspaceId, Long pageId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
