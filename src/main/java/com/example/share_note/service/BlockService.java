...
public class BlockService {
     ...
     * @return
     */
    public Mono<BlockCreateResponseDto> createBlock(Long workspaceId, Long pageId, BlockCreateRequestDto request) {
                        ...
                        reactivePageRepository.findByIdAndWorkspaceId(pageId, workspaceId)
                                ...
                                .flatMap(page -> {
                                    ...
                                    if (request.getParentBlockId() != null) {
                                        return reactiveBlockRepository.findByIdAndPageId(request.getParentBlockId(), pageId)
                                ...
                                .flatMap(page ->
                                        reactiveBlockRepository.save(Block.builder()
                                                .pageId(pageId)
                                                .parentBlockId(request.getParentBlockId())
     ...
     * @return
     */
    public Mono<BlockListResponseDto> getBlocks(Long workspaceId, Long pageId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
     ...
     * @return
     */
    public Mono<BlockResponseDto> getBlock(Long workspaceId, Long pageId, Long blockId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
     ...
     * @return
     */
    public Mono<BlockResponseDto> updateBlock(Long workspaceId, Long pageId, Long blockId, BlockUpdateRequestDto request) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
     ...
     * @return
     */
    public Mono<BlockResponseDto> moveBlock(Long workspaceId, Long pageId, Long blockId, BlockMoveRequestDto request) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                                ...
                                .flatMap(block ->
                                        reactivePageRepository.findByIdAndWorkspaceId(pageId, workspaceId)
                                                ...
                                                .flatMap(blockToMove -> {
                                                    ...
                                                    if (request.getNewParentBlockId() != null && request.getNewParentBlockId().equals(blockId)) {
                                                    ...
                                                    if (request.getNewParentBlockId() != null) {
                                                        return reactiveBlockRepository.findByIdAndPageId(request.getNewParentBlockId(), pageId)
                                                                .switchIfEmpty(Mono.error(new BlockException(ErrorCode.PARENT_BLOCK_NOT_FOUND)))
                                                ...
                                                .flatMap(blockToMove -> {
                                                    // 블록 위치 변경
                                                    blockToMove.setParentBlockId(request.getNewParentBlockId());
     ...
     * @return
     */
    public Mono<BlockStatusResponseDto> archiveBlock(Long workspaceId, Long pageId, Long blockId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                        reactiveBlockRepository.findByIdAndPageId(blockId, pageId)
                                ...
                                .flatMap(block ->
                                        ...
                                        reactiveBlockRepository.updateArchiveStatusForBlockTree(blockId, true, customUserDetails.getId())
                                                .thenReturn(BlockStatusResponseDto.builder().blockId(blockId).isArchived(true).build())
                                )
                )
                .onErrorMap(throwable -> {
                    if (throwable instanceof BlockException ||
     ...
     * @return
     */
    public Mono<BlockStatusResponseDto> restoreBlock(Long workspaceId, Long pageId, Long blockId) {
        return ReactiveSecurityContextHolder.getContext()
                .map(securityContext -> (CustomUserDetails) securityContext.getAuthentication().getPrincipal())
                .switchIfEmpty(Mono.error(new WorkspaceException(ErrorCode.AUTHENTICATION_FAILED)))
                .flatMap(customUserDetails ->
                        reactiveBlockRepository.findByIdAndPageId(blockId, pageId)
                                ...
                                .flatMap(block ->
                                        ...
                                        reactiveBlockRepository.updateArchiveStatusForBlockTree(blockId, false, customUserDetails.getId())
                                                .thenReturn(BlockStatusResponseDto.builder().blockId(blockId).isArchived(false).build())
                                )
                )
                .onErrorMap(throwable -> {
                    if (throwable instanceof BlockException ||
