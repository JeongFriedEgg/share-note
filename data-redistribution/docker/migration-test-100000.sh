...
generate_test_data() {
    ...
    log_info "Legacy DB에 테스트 데이터 생성 중 (페이지 100000개, 권한 300000개, 블록 3000000개)..."
    PGPASSWORD=$DB_PASSWORD psql -h localhost -p $LEGACY_PORT -U $DB_USER -d $LEGACY_DB -f test/04-test-data-100000.sql
...
monitor_migration_progress() {
    ...
    local total_pages=100000 # 시작 시점의 총 페이지 수
